		...
		switch (value) {
			...
			case AREA:
				...
				for (VisibleObject nextCreature : skill.getEffector().getKnownList().getKnownObjects().values()) {
					...
					if (Math.abs(skill.getEffector().getZ() - firstTarget.getZ()) > diffZ || ((firstTarget instanceof Player) && ((Player) firstTarget).isInPlayerMode(PlayerMode.WINDSTREAM))) {
						effectedList.remove(firstTarget);
						continue;
					}

					if (Math.abs(skill.getEffector().getZ() - nextCreature.getZ()) > diffZ || ((nextCreature instanceof Player) && ((Player) nextCreature).isInPlayerMode(PlayerMode.WINDSTREAM))) {
						continue;
					}
					...
					if (skill.isPointSkill()) {
						...
						// Lightning bolt
						if (MathUtil.isInsideAttackCylinder(skill.getEffector(), nextCreature, distance, properties.getEffectiveWidth(), !properties.isBackDirection())) {
						...
						if (!range.containsFloat(PositionUtil.getAngleToTarget(skill.getEffector(), nextCreature)))
							continue;
						if (MathUtil.isIn3dRange(skill.getEffector(), nextCreature, distance + firstTarget.getObjectTemplate().getBoundRadius().getCollision())) {
							if (!skill.shouldAffectTarget(nextCreature))
								continue;
							skill.getEffectedList().add((Creature) nextCreature);
						}
