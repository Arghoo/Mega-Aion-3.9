...
public class TriroansSummonAI2 extends AggressiveNpcAI2
{
		...
		if (boss != null && checkDistance() == 0 && !boss.getLifeStats().isAlreadyDead()) {
			if (!boss.isCasting())
				...
				ThreadPoolManager.getInstance().schedule(() -> {
					Npc boss1 = getPosition().getWorldMapInstance().getNpc(214669);
					if (boss1 != null && checkDistance() == 0 && !boss1.getLifeStats().isAlreadyDead())
						checkSkillUse(boss1);
				}, 5000);
			}
		}
	...
	{
		ThreadPoolManager.getInstance().schedule(() -> AI2Actions.deleteOwner(TriroansSummonAI2.this), 3000);
	}
}
