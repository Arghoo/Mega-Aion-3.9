...
{
	private final static Logger log = LoggerFactory.getLogger(PremiumDAO.class);
	...
	public static long getPoints(int accountId)
	{
		...
		try (Connection con = DatabaseFactory.getConnection();
			 PreparedStatement st = DB.prepareStatement("SELECT uniqId,points FROM account_rewards WHERE accountId=? AND rewarded=0")) {
			st.setInt(1, accountId);
			try (ResultSet rs = st.executeQuery()) {
				if (rs.next()) {
