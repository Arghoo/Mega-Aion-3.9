...
public class ChallengeTaskService
{
			...
			if (task.isCompleted()) {
				...
				for (Integer memberObjId : player.getLegion().getLegionMembers()) {
					...
					if (member != null && member.getLegionMember().getChallengeScore() > 0) {
						...
						LegionMember legionMember = LegionMemberDAO.loadLegionMember(memberObjId);
						...
						LegionMemberDAO.storeLegionMember(memberObjId, legionMember);
					}
				}
					...
					for (int objectId : e.getValue()) {
						for (ContributionReward reward : taskTemplate.getContrib()) {
							if (rewardsAdded <= reward.getNumber()) {
								...
								String recipientName = PlayerDAO.loadPlayerCommonData(objectId).getName();
	...
	public boolean canRaiseLegionLevel(int legionId, int legionLevel)
	{
		...
		if (legionTasks.containsKey(legionId)) {
			...
			tasks = ChallengeTasksDAO.load(legionId, ChallengeType.LEGION);
		}
