...
public class HousingBidService extends AbstractCronTask
{
	...
	protected void postInit()
	{
		...
		timeProlonged = ServerVariablesDAO.load("auctionProlonged");
	}
	...
	protected void executeTask()
	{
		...
		for (Entry<HouseBidEntry, Integer> winData : winners.entrySet()) {
			...
			if (LoggingConfig.LOG_HOUSE_AUCTION) {
				log.info("Address " + wonHouse.getAddress().getId() + " sold for price " + winData.getKey().getBidPrice() + " (bid count: " + winData.getKey().getBidCount() + "; result: " + result + ") to player " + winData.getKey()
					.getLastBiddingPlayer());
			}
		}
		...
		for (Entry<HouseBidEntry, Integer> sellData : successSell.entrySet()) {
			...
			if (LoggingConfig.LOG_HOUSE_AUCTION) {
				log.info("Address " + soldHouse.getAddress().getId() + " sold by player " + sellerPcd.getPlayerObjId() + " for price " + sellData.getKey().getBidPrice() + " (bid count: " + sellData.getKey().getBidCount() + "; result: " + result
					+ ") to player " + sellData.getKey().getLastBiddingPlayer());
			}
		}
	...
	{
		timeProlonged = 0;
		ServerVariablesDAO.store("auctionProlonged", timeProlonged);
	}
	...
	private PlayerCommonData getPlayerData(int objectId)
	{
		...
		if (player == null)
			return PlayerDAO.loadPlayerCommonData(objectId);
	...
	public boolean addHouseToAuction(House house, long initialPrice)
	{
		...
		house.save();
		return HouseBidsDAO.addBid(0, house.getObjectId(), initialPrice, time);
	}
	...
	public synchronized void placeBid(Player player, int entryIndex, long bidOffer)
	{
		...
		if (minutesLeft < 5 && timeProlonged < 30) {
			timeProlonged += 5;
			// save time
			ThreadPoolManager.getInstance().execute(() -> ServerVariablesDAO.store("auctionProlonged", timeProlonged));
		...
		if (bidOffer > entry.getBidPrice() || entry.getBidCount() == 0) {
			...
			HouseBidsDAO.addBid(player.getObjectId(), bidHouse.getObjectId(), bidOffer, time);
	...
	public void sendBidList(Player player)
	{
		int maxBidListPacketSize = 140;
		...
		// get list of house bids
		List<HouseBidEntry> houseBids = HousingBidService.getInstance().getHouseBidEntries(player.getRace());

		boolean isFirstPacket = true;
		do {
			List<HouseBidEntry> sendList = new ArrayList<HouseBidEntry>();
			int i = 0;

			for (HouseBidEntry entry : houseBids) {
				sendList.add(entry);

				// max letters per packet reached
				if (++i >= maxBidListPacketSize)
					break;
			}

			// remove sendList from bidList
			houseBids.removeAll(sendList);

			boolean isLastPacket = houseBids.isEmpty();

			PacketSendUtility.sendPacket(player, new SM_HOUSE_BIDS(player, sendList, playerBid, playerHouses, isFirstPacket, isLastPacket));
