...
public class EventManager
{
	...
	private EventRegisterInfo registerPlayerGroup(Player leader, EventType etype)
	{
		...
		for (Player member : group.getMembers()) {
			if (this.containsInHolders(member)) {
				return new EventRegisterInfo(EventRergisterState.ALREADY_REGISTRED, "One or more groups of players already registered in this event or other events.");
			}
		}
	...
	public EventRegisterInfo unregisterPlayerGroup(Player leader)
	{
		...
		if (holder != null) {
			if (holder.deletePlayerGroup(group)) {
				return new EventRegisterInfo(EventRergisterState.UNREGISTRED, String.format("Your group left the list of the registrar on Event: %s.", holder.getEventType().getEventTemplate().getEventName()));
			}
		}
	...
	private boolean addPlayerToPlayedMap(Player p, EventType etype)
	{
		if (this.playedPlayers.containsKey(etype)) {
			...
			log.info("[EventManager] Player: {} add to played list. Time in: {} Next time in: {}", new Object[] { p.getName(), new Timestamp(pi.lastEntryTime).toString(), new Timestamp(pi.nextEntryTime).toString() });
		} else {
