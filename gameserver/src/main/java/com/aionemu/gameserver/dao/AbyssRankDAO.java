...
public class AbyssRankDAO
{
	...
	public static final String UPDATE_PLAYER_RANK_LIST =
		"UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT id FROM players where race = ?) order by ap desc" + (RankingConfig.TOP_RANKING_SMALL_CACHE ? " limit 500"
			: "");  //only 300 positions are relevant later, so we update them + some extra positions that can get into the toprankings
	public static final String UPDATE_PLAYER_RANK_LIST_ACTIVE_ONLY =
		"UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT id FROM players where race = ? AND UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60) order by ap desc"
	...
	public static final String UPDATE_LEGION_RANK_LIST =
		"UPDATE legions SET legions.old_rank_pos = legions.rank_pos, legions.rank_pos = @a:=@a+1 where id in (SELECT legion_id FROM legion_members, players where rank = 'BRIGADE_GENERAL' AND players.id = legion_members.player_id and players.race = ?) order by legions.contribution_points DESC"
	...
	public static ArrayList<AbyssRankingResult> getAbyssRankingPlayers(final Race race, final int maxOfflineDays)
	{
	...
	public static Map<Integer, Integer> loadPlayersAp(final Race race, final int lowerApLimit, final int maxOfflineDays)
	{
		...
		try (Connection con = DatabaseFactory.getConnection();
			 PreparedStatement stmt = con.prepareStatement(maxOfflineDays > 0 ? SELECT_AP_PLAYER_ACTIVE_ONLY : SELECT_AP_PLAYER)) {
			stmt.setString(1, race.toString());
			...
			log.error("Couldn't load ranks for race " + race);
			return null;
		}
	...
	public static void updateRankList(final int maxOfflineDays)
	{
		try (Connection con = DatabaseFactory.getConnection();
	...
	private static int getLegionMembersCount(final int legionId)
	{
		...
		try (Connection con = DatabaseFactory.getConnection();
			 PreparedStatement ps = con.prepareStatement(SELECT_LEGION_COUNT)) {
			ps.setInt(1, legionId);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result[0] += rs.getInt("players");
			}
		} catch (SQLException e) {
			log.error("getLegionMembersCount", e);
		}
