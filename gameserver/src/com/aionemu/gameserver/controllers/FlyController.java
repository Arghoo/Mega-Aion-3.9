...
public class FlyController {
	...
	private static final long FLY_REUSE_TIME = 5000;
	...
	private ActionObserver hybridObserver = new ActionObserver(ObserverType.MOVE) {
		...
		public void moved() {
			if (!player.isInsideZoneType(ZoneType.FLY)) {
				...
				PacketSendUtility.broadcastPacket(player, new SM_EMOTION(player, EmotionType.GLIDE, 0, 0), true);
			}
		}
	};
	...
	public void onStopGliding(boolean removeWings) {
		if (player.isInState(CreatureState.GLIDING)) {
			...
			if (player.isInState(CreatureState.FLYING)) {
				...
				if (removeWings) {
					PacketSendUtility.broadcastPacket(player, new SM_EMOTION(player, EmotionType.RUN, 0, 0), true);
				}
			}
			//remove observer			
	...
	public boolean switchToGliding() {
		if (!player.isInState(CreatureState.GLIDING) && player.canPerformMove()) {
			...
			//add glideObserver
			if(player.isInsideZoneType(ZoneType.FLY)) {
				player.getObserveController().addObserver(hybridObserver);
			}
