...
public class DecomposeAction extends AbstractItemAction
{
	...
	private static final int USAGE_DELAY = TimingsConfig.DECOMPOSE_SPEED;
								...
								switch (randomItem.getType()) {
									...
									case MANASTONE_LEGEND_GRADE_70:
										...
										if (!randomType.equals(RandomType.MANASTONE)) {
											...
											ItemQuality finalItemQuality = itemQuality;
											List<ItemTemplate> selectedStones = stones.stream().filter(o -> o.getItemQuality().equals(finalItemQuality)).collect(Collectors.toList());
											randomId = selectedStones.get(Rnd.get(selectedStones.size())).getTemplateId();
										} else {
											List<ItemTemplate> selectedStones = stones.stream().filter(o -> !o.getItemQuality().equals(ItemQuality.LEGEND)).collect(Collectors.toList());
											randomId = selectedStones.get(Rnd.get(selectedStones.size())).getTemplateId();
										}
