...
public class EffectController
{
	...
	private boolean priorityStigmaEffect(Effect effect)
	{
		...
		while (iter2.hasNext()) {
			Effect ef = iter2.next();
			if (ef.getSkillTemplate().getStigmaType().getId() < effect.getSkillTemplate().getStigmaType().getId() && ef.getTargetSlot() == effect.getTargetSlot() && ef.getTargetSlotLevel() == effect.getTargetSlotLevel()) {
				for (EffectTemplate et : ef.getEffectTemplates()) {
					if (et.getEffectid() == 0)
						continue;
					for (EffectTemplate et2 : effect.getEffectTemplates()) {
