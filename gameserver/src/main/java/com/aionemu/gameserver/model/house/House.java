 ...
 * later version.
 *
 * mega-aion is distributed in the hope that it will be useful, but WITHOUT
...
public class House extends VisibleObject
{
	...
	public synchronized void spawn(int instanceId)
	{
		...
		if (playerObjectId > 0 && status == HouseStatus.ACTIVE || status == HouseStatus.SELL_WAIT) {
			PlayerRegisteredItemsDAO.loadRegistry(playerObjectId);
		}
		...
		if (playerObjectId != 0) {
			...
			Map<Integer, String> playerNames = PlayerDAO.getPlayerNames(players);
	...
	public synchronized boolean revokeOwner()
	{
		...
		if (this.getBuilding().getType() == BuildingType.PERSONAL_INS) {
			...
			HousesDAO.deleteHouse(playerObjectId);
			return true;
		}
		houseRegistry = null;
		acquiredTime = null;
