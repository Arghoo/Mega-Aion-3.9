...
public class LegionMemberDAO
{
	 ...
	 *
	 * @param playerObjId name to check
	 * @return true if name is used, false in other case
	 */
	public static boolean isIdUsed(final int playerObjId)
	{
		try (PreparedStatement s = DB.prepareStatement("SELECT count(player_id) as cnt FROM legion_members WHERE ? = legion_members.player_id")) {
	...
	public static ArrayList<Integer> loadLegionMembers(final int legionId)
	{
		...
		if (success && !legionMembers.isEmpty()) {
	 ...
	 *
	 * @param playerObjId legion member to delete
	 */
