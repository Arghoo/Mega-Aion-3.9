	...
	public static void openPrivateStore(Player activePlayer, String name)
	{
		...
		if (name != null) {
			...
			if (CustomConfig.SPEAKING_BETWEEN_FACTIONS) {
				...
				PacketSendUtility.broadcastPacket(playerActive, new SM_PRIVATE_STORE_NAME(playerActive.getObjectId(), name), true, p -> ((senderRace == p.getRace().getRaceId() && !p.getBlockList().contains(playerActive.getObjectId())) || p.isGM()));
				PacketSendUtility.broadcastPacket(playerActive, new SM_PRIVATE_STORE_NAME(playerActive.getObjectId(), ""), false, p -> senderRace != p.getRace().getRaceId() && !p.getBlockList().contains(playerActive.getObjectId()) && !p.isGM());
			}
		} else {
