...
public class AttackUtil
{
	 ...
	 * @param accMod
	 */
	public static void calculateSkillResult(Effect effect, int skillDamage, ActionModifier modifier, Func func, int randomDamage, int accMod, int criticalProb, int critAddDmg, boolean cannotMiss, boolean shared, boolean ignoreShield)
	{
		...
		if (randomDamage > 0) {
			...
			switch (randomDamage) {
				...
				default:
					/*
					 * chance to do from 100% to 200% damage This must NOT be calculated after critical status check, or it will be
					 * over powered and not retail
					 */
					damage *= (Rnd.get(50, 100) * 0.02f);
					break;
			}
		}
	...
	public static void calculateMagicalSkillResult(Effect effect, int skillDamage, ActionModifier modifier, SkillElement element, boolean useMagicBoost, boolean useKnowledge, boolean noReduce, Func func, int criticalProb, int critAddDmg,
		boolean shared, boolean ignoreShield)
	{
		Creature effector = effect.getEffector();
		Creature effected = effect.getEffected();
	...
	public static void calculateMagicalSkillResult2(Effect effect, int skillDamage)
	{
		Creature effector = effect.getEffector();
		Creature effected = effect.getEffected();

		AttackStatus status = calculateMagicalStatus(effector, effected, 0, true);
		calculateEffectResult(effect, effected, skillDamage, status, HitType.MAHIT, false);
	}
