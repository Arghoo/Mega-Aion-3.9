	...
	protected void writeImpl(LoginServerConnection con)
	{
		...
		switch (type) {
			...
			case CHARACTER_INFORMATION: {
				...
				//inventory
				List<Item> inv = InventoryDAO.loadStorageDirect(player.getObjectId(), StorageType.CUBE);
				writeD(inv.size());
				...
				inv = InventoryDAO.loadStorageDirect(player.getObjectId(), StorageType.REGULAR_WAREHOUSE);
				ItemService.loadItemStones(inv);
				writeD(inv.size());
				for (Item item : inv) {
					writeD(item.getObjectId());
				...
				writeD(macro.getMacros().size());
				for (Entry<Integer, String> m : macro.getMacros().entrySet()) {
					writeD(m.getKey());
					writeS(m.getValue());
				}
