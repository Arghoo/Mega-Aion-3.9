...
public class cmd_transfer extends PlayerCommand
{
	...
	public void execute(Player player, String... params)
	{
		...
		// chk player name
		if (PlayerDAO.loadPlayerCommonDataByName(accountOwnerName) == null) {
			PacketSendUtility.sendMessage(player, "Wrong player name.");
			return;
		}

		// get account id by name
		int accountId = PlayerDAO.getAccountIdByName(accountOwnerName);
		...
		List<Integer> playersIds = PlayerDAO.getPlayerOidsOnAccount(accountId);
		...
		PlayerDAO.changePlayerId(player, accountId);
