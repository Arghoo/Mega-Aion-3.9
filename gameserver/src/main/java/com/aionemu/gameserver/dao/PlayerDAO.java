...
public class PlayerDAO extends IDFactoryAwareDAO
{
	...
	public static void changePlayerId(final Player player, final int accountId)
	{
		try (Connection con = DatabaseFactory.getConnection();
	...
	public static void storePlayer(final Player player)
	{
		try (Connection con = DatabaseFactory.getConnection();
			 PreparedStatement stmt = con.prepareStatement(
				 "UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, last_online=?, quest_expands=?, npc_expands=?, advanced_stigma_slot_size=?, warehouse_size=?, note=?, title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, mentor_flag_time=?, world_owner=? WHERE id=?")) {
			...
			if (player.getPosition().getWorldMapInstance() == null) {  //FIXME!
				log.error("Error saving player: " + player.getObjectId() + " " + player.getName() + ", world map instance is null. Setting world owner to 0. Position: " + player.getWorldId() + " " + player.getX() + " " + player.getY() + " "
					+ player.getZ());
				stmt.setInt(24, 0);
			} else {
	 ...
	 * @return List of IDs
	 */
	public static Set<Integer> getInactiveAccounts(final int daysOfInactivity, final int limitation)
	{
		...
		//at least one active char on account
		inactiveAccounts.entrySet().removeIf(entry -> entry.getValue() < getCharacterCountOnAccount(entry.getKey()));
	...
	{
		return DB.getUsedIDs("players", "id");
	}
}
