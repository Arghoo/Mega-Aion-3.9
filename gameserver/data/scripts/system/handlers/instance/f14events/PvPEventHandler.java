...
//        }
	...
	public void onEnterInstance(Player player)
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> StartRoundTask(), waitingTime * 1000);
	...
	private synchronized void StartRoundTask()
	{
		if (endRoundTask == null) {
			...
			endRoundTask = ThreadPoolManager.getInstance().schedule(() -> NextRound(true), battle_time * 1000);
	...
	private void NextRound(boolean timeIsUp)
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> StartRoundTask(), this.delayBeforStartNextRound * 1000);
	}
	...
	protected void deathPlayer(Player victim, Creature lastAttacker)
	{
		if (lastAttacker.getActingCreature() instanceof Player && victim != lastAttacker) {
			...
			ThreadPoolManager.getInstance().schedule(() -> {
				...
				nextRoundTask = ThreadPoolManager.getInstance().schedule(() -> NextRound(false), 4000);
			}, 5000);
		}
	}
	...
	protected void DoReward()
	{
		if (!eventIsComplete) {
			...
			if (!isDraw) {
				...
				Balalaka.sayInWorldOrangeTextCenter(EventManager, String.format("Ивент: %s завершен, победил(а): %s, проиграл(а): %s", eType.getEventTemplate().getEventName(), names));
			} else {
