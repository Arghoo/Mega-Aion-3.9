...
public class ItemService
{
	...
	public static long addItem(Player player, int itemId, long count, Item sourceItem, ItemAddPredicate predicate)
	{
		...
		if (LoggingConfig.LOG_ITEM) {
			log.info("[ITEM] ID/Count" + (LoggingConfig.ENABLE_ADVANCED_LOGGING ? "/Item Name - " + itemTemplate.getTemplateId() + "/" + count + "/" + itemTemplate.getName() : " - " + itemTemplate.getTemplateId() + "/" + count) + " to player "
				+ player.getName());
		}
