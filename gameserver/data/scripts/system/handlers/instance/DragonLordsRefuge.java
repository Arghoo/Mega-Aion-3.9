	...
	public void onDie(Npc npc)
	{
		...
		switch (npcId) {
			...
			case 219407: //Calindi Flamelord
				...
				ThreadPoolManager.getInstance().schedule(() -> performSkillToTarget(219409, 219409, 20920), 8000);

				//spawn Kaisinel or Marchutan Gods (depends of group race)
				ThreadPoolManager.getInstance().schedule(() -> spawn((instanceRace == Race.ELYOS ? 219564 : 219567), 504f, 515f, 417.405f, (byte) 60), 15000);
			...
			ThreadPoolManager.getInstance().schedule(() -> {
				if (isSpawned(npcId + 1)) {
					...
					ThreadPoolManager.getInstance().schedule(() -> {
						...
						despawnNpc(219409);
						sendMsg(msg);
						ThreadPoolManager.getInstance().schedule(() -> startFinalTimer(), 10000);
					}, 2000);
				}
			}, 40000);
		}
