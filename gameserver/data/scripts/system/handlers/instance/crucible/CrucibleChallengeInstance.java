...
public class CrucibleChallengeInstance extends CrucibleInstance
{
	...
	public void onDie(final Npc npc)
	{
		...
		switch (npc.getNpcId()) {
			...
			case 217845:
				...
				ThreadPoolManager.getInstance().schedule(() -> despawnNpcs(getNpcs(217848)), 3000);
				break;
			case 217847:
				despawnNpc(npc);
			...
			case 217842:
				...
				if (getNpcs(fnpcId).size() == 2) {
					ThreadPoolManager.getInstance().schedule(() -> startStage3Round1_1(fnpcId), 5000);
	...
	public void doReward(Player player)
	{
		...
		float reward = 0.03f * playerReward.getPoints() + 150;
	...
	private void startBonusStage2()
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> setEvent(StageType.START_BONUS_STAGE_2, 1000), 2000);
