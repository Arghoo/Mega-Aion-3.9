...
public class SourceSiege extends Siege<SourceLocation>
{
	...
	protected void onSiegeFinish()
	{
		if (LoggingConfig.LOG_SIEGE) {
			...
			if (winner != null)
				log.info(
					"[SIEGE] > Siege finished. [SOURCE:" + getSiegeLocationId() + "] [OLD RACE: " + getSiegeLocation().getRace() + "] [OLD LegionId:" + getSiegeLocation().getLegionId() + "] [NEW RACE: " + winner.getSiegeRace() + "] [NEW LegionId:"
	...
	protected void giveRewardsToPlayers(SiegeRaceCounter winnerDamage)
	{
		...
		for (SiegeReward topGrade : playerRewards) {
			...
			for (int rewardedPC = 0; i < topPlayersIds.size() && rewardedPC < topGrade.getTop(); ++i) {
				...
				++rewardedPC;
				if (LoggingConfig.LOG_SIEGE) {
					log.info("[SIEGE]  > [SOURCE:" + getSiegeLocationId() + "] [RACE: " + getSiegeLocation().getRace() + "] Player Reward to: " + playerNames.get(playerId) + "] ITEM RETURN " + topGrade.getItemId() + " ITEM COUNT "
