...
import com.aionemu.gameserver.dao.PlayerMacrosDAO;
...
public class PlayerService
{
	...
	{
		return PlayerDAO.saveNewPlayer(player.getCommonData(), accountId, accountName) && PlayerAppearanceDAO.store(player) && PlayerSkillListDAO.storeSkills(player) && InventoryDAO.store(player);
	}
	...
	public static void storePlayer(Player player)
	{
		...
		for (House house : player.getHouses()) {
			HousesDAO.storeHouse(house);
			if (house.getRegistry() != null && house.getRegistry().getPersistentState() == PersistentState.UPDATE_REQUIRED) {
				PlayerRegisteredItemsDAO.store(house.getRegistry(), player.getCommonData().getPlayerObjId());
			}
		}
	...
	public static Player getPlayer(int playerObjId, Account account)
	{
		...
		MacroList macroses = PlayerMacrosDAO.restoreMacros(playerObjId);
