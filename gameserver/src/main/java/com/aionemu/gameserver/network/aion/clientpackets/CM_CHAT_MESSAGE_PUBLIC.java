...
public class CM_CHAT_MESSAGE_PUBLIC extends AionClientPacket
{
	...
	{
		PacketSendUtility.broadcastPacket(player, new SM_MESSAGE(player, message, type), true, p -> !p.getBlockList().contains(player.getObjectId()));
	}
	...
	private void broadcastToNonBlockedRacePlayers(final Player player)
	{
		...
		PacketSendUtility.broadcastPacket(player, new SM_MESSAGE(player, message, type), true, p -> ((senderRace == p.getRace().getRaceId() && !p.getBlockList().contains(player.getObjectId())) || p.isGM()));
		PacketSendUtility.broadcastPacket(player, new SM_MESSAGE(player, "Unknow Message", type), false, p -> (senderRace != p.getRace().getRaceId() && !p.getBlockList().contains(player.getObjectId()) && !p.isGM()));
	}
