...
public class GravityCrusherAI2 extends AggressiveNpcAI2
{
	...
	protected void handleSpawned()
	{
		super.handleSpawned();
		transform();
		ThreadPoolManager.getInstance().schedule(() -> attackPlayer(), 2000);
	}

	private void transform()
	{
		transformTask = ThreadPoolManager.getInstance().schedule(() -> {
			if (!isAlreadyDead()) {
				if (skillTask != null)
	...
	public void handleMoveArrived()
	{
		...
		skillTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() -> AI2Actions.useSkill(GravityCrusherAI2.this, 20987), 0, 2000);
	}
