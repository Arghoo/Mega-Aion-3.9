...
public class FFAEventHandler extends BaseEventHandler
{
	...
	public void onInstanceCreate(WorldMapInstance instance)
	{
		...
		this.waitingTask = ThreadPoolManager.getInstance().schedule(() -> StartRoundTask(), this.waitingTime * 1000);
	}
	...
	private void StartRoundTask()
	{
		if (endRoundTask == null) {
			...
			endRoundTask = ThreadPoolManager.getInstance().schedule(() -> EndBattle(), battle_time * 1000);
	...
	protected void DoReward()
	{
		if (!eventIsComplete) {
			...
			Balalaka.sayInWorldOrangeTextCenterWithDelay(EventManager, String.format("Ивент: %s завершен, первое место занял(а): %s .", eType.getEventTemplate().getEventName(), this.getPlayerFromEventList(this.score.get(0).PlayerObjectId).getName()),
				3);
