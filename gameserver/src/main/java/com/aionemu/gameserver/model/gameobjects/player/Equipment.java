...
public class Equipment
{
	...
	private Item equip(long itemSlotToEquip, Item item)
	{
		synchronized (equipment) {
			...
			if (equipment.get(itemSlotToEquip) != null) {
				log.error("CHECKPOINT : putting item to already equiped slot. Info slot: " + itemSlotToEquip + " new item: " + item.getItemTemplate().getTemplateId() + " old item: " + equipment.get(itemSlotToEquip).getItemTemplate().getTemplateId());
				return null;
			}
	...
	private boolean validateEquippedWeapon(Item item, boolean validateOnly)
	{
		...
		switch (item.getItemTemplate().getWeaponType().getRequiredSlots()) {
			...
			case 1:
				// check dual skill
				if (itemInMainHand != null && (!owner.getSkillList().isSkillPresent(19) && !owner.getSkillList().isSkillPresent(360) && !owner.getSkillList().isSkillPresent(127) && !owner.getSkillList().isSkillPresent(128) && !owner.getSkillList()
					.isSkillPresent(924))) {
					if (validateOnly) {
						requiredSlots++;
	...
	public long increaseEquippedItemCount(Item item, long count)
	{
		...
		if (item.getItemTemplate().getCategory() != ItemCategory.SHARD && item.getItemTemplate().getArmorType() != ArmorType.ARROW)
			return count;
	...
	private void decreaseEquippedItemCount(int itemObjId, int count)
	{
		...
		if (equippedItem.getItemTemplate().getCategory() != ItemCategory.SHARD && equippedItem.getItemTemplate().getArmorType() != ArmorType.ARROW)
			return;
