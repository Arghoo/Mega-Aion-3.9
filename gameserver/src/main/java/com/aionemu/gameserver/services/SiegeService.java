...
public class SiegeService
{
	...
	public void initSieges()
	{
		...
		CronService.getInstance().schedule(() -> {
			if (moltenusAbyssBoss.containsKey(251045) && moltenusAbyssBoss.get(251045).isSpawned())
				...
				World.getInstance().doOnAllPlayers(player -> PacketSendUtility.sendBrightYellowMessage(player, "Menotios: Fragment of the anger has been sighted in the Abyss"));
				...
				ThreadPoolManager.getInstance().schedule(() -> {
					for (VisibleObject vo : moltenusAbyssBoss.values()) {
						...
						World.getInstance().doOnAllPlayers(player -> PacketSendUtility.sendBrightYellowMessage(player, "Menotios: Fragment of anger gone"));
					}
				}, 3600 * 1000);
			}
	...
	public void startPreparations()
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> {
			...
			World.getInstance().getWorldMap(600040000).getWorldMapInstance().doOnAllPlayers(player -> TeleportService2.moveToBindLocation(player, true));
		...
		ThreadPoolManager.getInstance().schedule(() -> {
			...
			World.getInstance().getWorldMap(600030000).getWorldMapInstance().doOnAllPlayers(player -> {
				for (SourceLocation source : getSources().values())
		...
		for (final SourceLocation source : getSources().values()) {
			...
			if (!source.getRace().equals(SiegeRace.BALAUR)) {
				...
				SiegeDAO.updateSiegeLocation(source);
			}
		}
