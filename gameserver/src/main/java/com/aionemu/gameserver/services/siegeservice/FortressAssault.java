...
public class FortressAssault extends Assault<FortressSiege>
{
	...
	protected void scheduleAssault(int delay)
	{
		dredgionTask = ThreadPoolManager.getInstance().schedule(() -> {
			...
			spawnTask = ThreadPoolManager.getInstance().schedule(this::spawnAttackers, Rnd.get(240, 300) * 1000);
		}, delay * 1000);
	}
	...
	protected void onAssaultFinish(boolean captured)
	{
		...
		if (!captured)
			...
			siegeLocation.doOnAllPlayers(player -> PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_ABYSS_DRAGON_BOSS_KILLED));
		}
	}
	...
	private void spawnAttackers()
	{
		...
		int amount = isBalaurea ? Rnd.get(10, 20) : Rnd.get(10, 20);
		int templateId;
		...
		siegeLocation.doOnAllPlayers(player -> PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_ABYSS_CARRIER_DROP_DRAGON));
