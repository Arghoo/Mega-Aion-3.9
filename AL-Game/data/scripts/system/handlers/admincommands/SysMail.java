...
public class SysMail extends AdminCommand {
	...
	public void execute(Player admin, String... params) {
		...
		if (paramValues[1].startsWith("@")) {
			if ("@all".startsWith(paramValues[1]))
				recipientType = RecipientType.ALL;
			else if ("@elyos".startsWith(paramValues[1]))
				recipientType = RecipientType.ELYOS;
			else if ("@asmodians".startsWith(paramValues[1]))
			...
			recipient = Util.convertName(paramValues[1]);
		}
		...
		try {
			letterType = LetterType.getLetterTypeById(Integer.parseInt(paramValues[2]));
			item = Integer.parseInt(paramValues[3]);
			count = Integer.parseInt(paramValues[4]);
			kinah = Integer.parseInt(paramValues[5]);
		}
