...
public class ItemStorage
{
	...
	{
		return items.values().stream().filter(o -> o.getItemTemplate().getExtraInventoryId() > 0).collect(Collectors.toList());
	}

	public List<Item> getCubeItems()
	{
		return items.values().stream().filter(o -> o.getItemTemplate().getExtraInventoryId() < 1).collect(Collectors.toList());
	}
