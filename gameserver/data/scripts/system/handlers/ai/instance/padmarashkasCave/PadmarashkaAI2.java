...
public class PadmarashkaAI2 extends AggressiveNpcAI2
{
	...
	{
		mainSkillTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(() -> SkillEngine.getInstance().getSkill(getOwner(), Rnd.get(2) == 0 ? 20401 : 20093, 48, getTarget()).useNoAnimationSkill(), 10000, 20000);
	}
	...
	private void scheduleDelayStage2(int delay)
	{
		if (!isStart && !isAlreadyDead())
			...
			ThreadPoolManager.getInstance().schedule(() -> stage2(), delay);
		}
	}
	...
	private void scheduleDelayStage1(int delay)
	{
		if (!isStart && !isAlreadyDead())
			...
			ThreadPoolManager.getInstance().schedule(() -> stage1(), delay);
		}
	}
	...
	private void scheduleDelayEntrance(int delay)
	{
		if (!isStart && !isAlreadyDead())
			...
			ThreadPoolManager.getInstance().schedule(() -> scheduleSpawnEntrance(), delay);
		}
	}
