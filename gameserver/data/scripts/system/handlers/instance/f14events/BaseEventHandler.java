...
*/
	...
	protected void AddProtection(final Player p, int duration)
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> RemoveProtection(p), duration);
	}
	...
	protected void AddProtection(final Player p, final int duration, int delay)
	{
		if (delay == 0) {
			...
			ThreadPoolManager.getInstance().schedule(() -> AddProtection(p, duration), delay);
		}
	}
	...
	protected final void sendSpecMessage(final String sender, final String msg, int delay)
	{
		if (delay == 0) {
			...
			ThreadPoolManager.getInstance().schedule(() -> sendSpecMessage(sender, msg), delay * 1000);
		}
	}
	...
	protected final void sendSpecMessage(final String sender, final String msg, final Player target, int delay)
	{
		if (delay == 0) {
			...
			ThreadPoolManager.getInstance().schedule(() -> sendSpecMessage(sender, msg, target), delay * 1000);
		}
	}
	...
	{
		ThreadPoolManager.getInstance().schedule(() -> TeleportService2.moveToBindLocation(p, true), 5000);
	}
