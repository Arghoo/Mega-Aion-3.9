...
public class CommandEventHandler extends BaseEventHandler
{
	...
	public void onInstanceCreate(WorldMapInstance instance)
	{
		...
		this.waitingTask = ThreadPoolManager.getInstance().schedule(() -> startBattle(), this.waitingTime * 1000);
	}
	...
	private void startBattle()
	{
		if (!this.battleIsStart) {
			...
			this.waitingTask = null;
			this.stopTimer();
			this.endTask = ThreadPoolManager.getInstance().schedule(() -> endBattle(null), this.battle_time * 1000);
	...
	private synchronized void endBattle(PlayerGroup winner)
	{
		if (!this.eventIsComplete) {
			...
			Balalaka.sayInWorldOrangeTextCenter(EventManager, String.format("Ивент: %s завершен, победила группа с лидером группы: %s.", this.eType.getEventTemplate().getEventName(), winner.getLeaderObject().getName()));
			this.DoReward();
		}
	}
