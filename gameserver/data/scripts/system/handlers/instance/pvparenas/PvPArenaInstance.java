...
public class PvPArenaInstance extends GeneralInstanceHandler
{
	...
	{
		for (Player player : instance.getPlayersInside()) {
			boolean isSolo = instanceReward.isSoloArena();
			PvPArenaPlayerReward reward = getPlayerReward(player.getObjectId());
			float playerRate = isSolo ? player.getRates().getDisciplineRewardRate() : player.getRates().getHarmonyRewardRate();
			if (!reward.isRewarded()) {
				reward.setRewarded();
				AbyssPointsService.addAp(player, reward.getBasicAP() + reward.getRankingAP() + (int) (reward.getScoreAP() * playerRate));
				...
				int crucible = reward.getBasicCrucible() + reward.getRankingCrucible() + (int) (reward.getScoreCrucible() * playerRate);
				if (crucible != 0) {
