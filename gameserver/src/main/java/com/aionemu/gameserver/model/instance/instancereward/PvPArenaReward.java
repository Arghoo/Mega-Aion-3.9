...
public class PvPArenaReward extends InstanceReward<PvPArenaPlayerReward>
{
	...
	{
		int minValue = Collections.min(getInstanceRewards(), Comparator.comparingInt(PvPArenaPlayerReward::getPoints)).getPoints();
		int maxValue = Collections.max(getInstanceRewards(), Comparator.comparingInt(PvPArenaPlayerReward::getPoints)).getPoints();
		if (isSoloArena() && (maxValue - minValue) >= 1500) {
			return true;
		}
		return maxValue >= capPoints;
	}
