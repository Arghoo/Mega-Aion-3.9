...
public class Player extends Creature
{
		...
		if (!enemy.getRace().equals(getRace())) {
			if (worldId == 600020000 || worldId == 600030000) {
				if (DisputeLandService.getInstance().isActive()) {
					...
					return (this.isInsideZoneType(ZoneType.PVP) && enemy.isInsideZoneType(ZoneType.PVP) && !this.isInDisablePvPZone() && !enemy.isInDisablePvPZone());
				}
	...
	public boolean isAggroFrom(Npc npc)
	{
		...
		return (isAggroIconTo(npc) && (npc.getTribe().isGuard() || npc.getObjectTemplate().getAbyssNpcType() != AbyssNpcType.NONE || npc.getLevel() + 10 > getLevel() || (npc.isInInstance() && InstanceService.isAggro(npc.getWorldId()))));
	}
	...
	public boolean isAggroIconTo(Npc npc)
	{
		...
		switch (getTribe()) {
			case PC:
				if (tribe == TribeClass.XDRAKAN_DGUARD || race == Race.ASMODIANS || (tribe != null ? tribe.isDarkGuard() : true) || race == Race.CONSTRUCT && tribe == TribeClass.PET_DARK) {
					return true;
				}
				return DataManager.TRIBE_RELATIONS_DATA.isAggressiveRelation(tribe, TribeClass.PC);
			case PC_DARK:
				if (tribe == TribeClass.XDRAKAN_LGUARD || race == Race.ELYOS || (tribe != null ? tribe.isLightGuard() : true) || race == Race.CONSTRUCT && tribe == TribeClass.PET) {
					return true;
				}
