...
public class AutoDredgionInstance extends AutoInstance {
	...
	public AGQuestion addPlayer(Player player, SearchInstance searchInstance) {
		...
		try {
			...
			if (ert.isGroupEntry()) {
				if(searchInstance.getMembers().size() == 4) {
					clearGroupByRace(playersByRace);
				...
				else if (searchInstance.getMembers().size() + playersByRace.size() > 4) {
					resetFailed(player);
					return AGQuestion.FAILED;
				}
				for (Player member : player.getPlayerGroup2().getOnlineMembers()) {
	...
	private void clearGroupByRace(List<AGPlayer> playerList) {
		 for (AGPlayer agp : playerList) {
			Integer obj = agp.getObjectId();
			if (players.containsKey(obj)) {
				Player player = World.getInstance().findPlayer(obj);
				players.remove(obj);
	...
	private void resetFailed(Player player) {
