...
public class EmpyreanCrucibleInstance extends CrucibleInstance
{
	...
	{
		this.stageType = type;
		ThreadPoolManager.getInstance().schedule(() -> instance.doOnAllPlayers(player -> PacketSendUtility.sendPacket(player, new SM_INSTANCE_STAGE_INFO(2, type.getId(), type.getType()))), time);
	}
		...
		switch (npc.getNpcId()) {
			...
			case 217528:
				...
				if (getNpcs(217527).isEmpty() && getNpcs(217528).isEmpty()) {
					...
					ThreadPoolManager.getInstance().schedule(() -> startBonusStage3(), 39000);
				}
				break;
			case 217557:
			case 217559:
			...
			case 217653:
				...
				if (isDoneStage4 && getNpcs(217651).isEmpty() && getNpcs(217652).isEmpty() && getNpcs(217653).isEmpty()) {
					...
					ThreadPoolManager.getInstance().schedule(() -> startBonusStage4(), 33000);
				}
				break;
			case 217547:
			case 217548:
	...
	public void doReward(Player player)
	{
		...
		float reward = 0.05f * playerReward.getPoints() + 350;
		if (!playerReward.isRewarded()) {
