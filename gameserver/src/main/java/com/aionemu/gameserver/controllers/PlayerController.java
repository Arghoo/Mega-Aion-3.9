...
public class PlayerController extends CreatureController<Player>
{
	...
	public void updateNearbyQuests()
	{
		...
		for (int questId : getOwner().getPosition().getMapRegion().getParent().getQuestIds()) {
			if (QuestService.checkStartConditions(new QuestEnv(null, getOwner(), questId, 0), false) && QuestService.checkLevelRequirement(questId, getOwner().getCommonData().getLevel())) {
	...
	public void startProtectionActiveTask()
	{
		if (!getOwner().isProtectionActive()) {
			...
			Future<?> task = ThreadPoolManager.getInstance().schedule(this::stopProtectionActiveTask, 60000);
