...
public class TeleportService2
{
	...
	public static boolean teleportTo(Player player, int worldId, int instanceId, float x, float y, float z, boolean isSkillLoci)
	{
		return teleportTo(player, worldId, instanceId, x, y, z, player.getHeading(), TeleportAnimation.NO_ANIMATION, isSkillLoci);
	}
	...
	// Loci Variant
	public static boolean teleportTo(final Player player, final int worldId, final int instanceId, final float x, final float y, final float z, final byte heading, TeleportAnimation animation, boolean isSkillLoci)
	{
		if (player.getLifeStats().isAlreadyDead())
			return false;
		else if (DuelService.getInstance().isDueling(player.getObjectId())) {
		...
		if (animation.isNoAnimation()) {
			player.unsetPlayerMode(PlayerMode.RIDE);
			changePosition(player, worldId, instanceId, x, y, z, heading, animation, isSkillLoci);
		} else {
			sendLoc(player, worldId, instanceId, x, y, z, heading, animation);
		}
		return true;
	...
	//Loci Variant
	private static void changePosition(Player player, int worldId, int instanceId, float x, float y, float z, byte heading, TeleportAnimation animation, boolean isSkillLoci)
	{
		if (player.hasStore()) {
			PrivateStoreService.closePrivateStore(player);
		}
