...
public class LHEventHandler extends BaseEventHandler
{
	...
	private void StartPreRoundTask(final Player player)
	{
		...
		ScheduledFuture task = ThreadPoolManager.getInstance().schedule(() -> StartRoundTask(), waitingTime * 1000);
	...
	protected void DoReward()
	{
		if (!eventIsComplete) {
			...
			Balalaka.sayInWorldOrangeTextCenterWithDelay(EventManager, String.format("Ивент: %s завершен, победитель: %s.", eType.getEventTemplate().getEventName(), winner.getName()), 2);
