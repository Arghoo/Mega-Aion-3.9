...
public class RentusBaseInstance extends GeneralInstanceHandler
{
	...
	public void onDie(final Npc npc)
	{
		...
		switch (npc.getNpcId()) {
			...
			case 217307:
				...
				instance.doOnAllPlayers(player -> removeEffects(player));
				startZantarazEvent();
				break;
			case 217313:
			...
			case 217299:
				...
				ThreadPoolManager.getInstance().schedule(() -> despawnNpc(npc), 2000);
				break;
		}
	}
	...
	private void startZantarazEvent()
	{
		...
		ThreadPoolManager.getInstance().schedule(() -> {
			...
			invisible.getKnownList().doOnAllPlayers(player -> PacketSendUtility.sendPacket(player, new SM_PLAY_MOVIE(0, 0, 482, 65536, invisible.getObjectId())));
			stopWalk(priest1);
			stopWalk(priest2);
			stopWalk(warrior1);
			stopWalk(warrior2);
