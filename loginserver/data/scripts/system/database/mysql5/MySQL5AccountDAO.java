...
public class MySQL5AccountDAO extends AccountDAO {
	...
	public boolean updateAccount(Account account) {
		int result = 0;
		PreparedStatement st = DB
			.prepareStatement("UPDATE account_data SET `name` = ?, `password` = ?, access_level = ?, membership = ?, last_server = ?, last_ip = ?, last_mac = ?, ip_force = ?, membershipExpiry = ? WHERE `id` = ?");
	...
	public boolean updateMembership(final int accountId) {
		return DB.insertUpdate(
			"UPDATE account_data SET membership = old_membership, membershipExpiry = NULL WHERE id = ? and membershipExpiry < CURRENT_TIMESTAMP",
			new IUStH() {
